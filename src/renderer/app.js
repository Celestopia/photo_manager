/**
 * Renderer entry (Vue, no bundler).
 *
 * This file drives both:
 * - Electron mode (real IPC bridge from preload)
 * - Browser preview mode (mock bridge from browser-api.js)
 */
const { createApp, reactive, ref, computed, onMounted, onBeforeUnmount, watch, nextTick } = Vue;

// Unified bridge object used by all UI actions.
...
const ICONS = {
  ...
  windowMinimize: "./assets/window_minimize.svg",
  windowMaximize: "./assets/window_maximize.svg",
  windowRestore: "./assets/window_restore.svg",
  windowClose: "./assets/window_close.svg",
  metadataInfo: "./assets/metadata_info.svg",
  zoomIn: "./assets/image_zoom_in.svg",
  zoomOut: "./assets/image_zoom_out.svg",
  rotateClockwise: "./assets/image_rotate_clockwise.svg",
  rotateCounterclockwise: "./assets/image_rotate_counterclockwise.svg",
  mirror: "./assets/image_mirror.svg",
  restoreView: "./assets/image_restore_view.svg",
  fullscreen: "./assets/image_fullscreen.svg",
  customization: "./assets/customization.svg",
};
          ...
          <aside class="side-panel right-panel" :class="{ collapsed: !showRightPanel }">
            ...
            <div class="location-field">
              <div class="location-label-row">
                <label>位置</label>
                <button type="button" class="location-info-btn" @click.stop="toggleLocationInfoMenu">!</button>
              </div>
              <div class="location-info-menu" v-if="showLocationInfoMenu" @click.stop>
                <div>国家：{{ selectedItem?.Location?.Country || '-' }}</div>
                <div>省区：{{ selectedItem?.Location?.Province || '-' }}</div>
                <div>城市：{{ selectedItem?.Location?.City || '-' }}</div>
              </div>
              <textarea class="input field-textarea" v-model="editDraft.LocationSite" @input="onFieldTextareaInput($event, 'LocationSite')" rows="1" placeholder="输入具体地点"></textarea>
            </div>
