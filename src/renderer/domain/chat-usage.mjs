...
export function sessionUsage(messages) {
  const attempts = messages.filter(m => m.role === 'assistant' && m.attempt).flatMap(m => m.attempt.steps.filter(s => s.kind === 'completion'));
  const usage = {};
  let incomplete = false;
  for (const [key] of usageFields) {
    const values = attempts.map(m => m.usage?.[key]).filter(v => v != null);
...
export const messageUsage = message => sessionUsage([message]);
