<template>
  <div class="gps-map-surface" @keydown.stop>
    <div ref="container" class="gps-map-canvas" aria-label="Media GPS map"></div>
    <button type="button" class="btn icon-btn gps-map-recenter" aria-label="Recenter" :data-tip="expanded ? undefined : 'Recenter'" :title="expanded ? 'Recenter' : undefined" @click="recenter"><svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><circle cx="12" cy="12" r="6"/><path d="M12 2v4m0 12v4M2 12h4m12 0h4"/><circle cx="12" cy="12" r="1.5" fill="currentColor" stroke="none"/></svg></button>
...
onMounted(() => {
  ...
  map = L.map(container.value, { attributionControl: false, scrollWheelZoom: props.expanded, minZoom: 1, maxZoom: 19 }).setView(center, props.viewport?.zoom ?? 15);
...
.gps-map-surface .leaflet-control-zoom a { color: var(--text); }
.gps-map-recenter, .gps-map-expand, .gps-map-attribution, .gps-map-status { position: absolute; z-index: 1000; }
.gps-map-recenter { bottom: 26px; left: 10px; }
.gps-map-expand { right: 10px; top: 10px; }
