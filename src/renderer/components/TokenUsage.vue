...
function key(event) { if (event.key === 'Escape' && open.value) { event.preventDefault(); event.stopPropagation(); close(); } }
async function toggle(event) {
  if (open.value) return close();
  window.dispatchEvent(new Event('close-token-usage'));
  portalTarget.value = event.currentTarget.closest('dialog') || document.body;
