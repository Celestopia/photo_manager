...
function createModelAssets(root) {
  ...
  function directory(model) {
    return path.join(root, model.repository, model.revision);
  }
  async function valid(file, entry) {
    try {
      ...
      return (
        ...
        !s.isSymbolicLink() &&
        s.size === entry.size &&
        (await digest(file)) === entry.sha256
      );
    } catch {
      return false;
    }
    ...
    return Promise.all(
      manifest.models.map(async (m) => {
        ...
        for (const entry of m.files)
          if (await valid(path.join(directory(m), entry.path), entry))
            installed++;
        return {
          id: m.id,
          ...
          bytes: m.files.reduce((n, e) => n + e.size, 0),
          ready: installed === m.files.length,
          installed,
  ...
  async function install({ signal, onProgress = () => {}, importRoot } = {}) {
    ...
    try {
      ...
      if (space.bavail * space.bsize < 1024 ** 3)
        throw new Error("At least 1 GiB free disk is required");
      for (const model of manifest.models)
        for (const entry of model.files) {
          ...
          if (importRoot) {
            ...
            if (!(await valid(source, entry)))
              throw new Error(
                `Invalid imported model artifact: ${model.id}/${entry.path}`,
              );
            ...
            if (offset < entry.size) {
              const response = await fetch(
                `https://huggingface.co/${model.repository}/resolve/${model.revision}/${entry.path}`,
                {
                  signal: combined,
              ...
              try {
                for await (const chunk of response.body) {
                  ...
                  offset += chunk.length;
                  if (offset > entry.size)
                    throw new Error("Model artifact exceeds pinned size");
                  let written = 0;
                  while (written < chunk.length) {
                    ...
                    if (!result.bytesWritten)
                      throw new Error("Model cache write made no progress");
