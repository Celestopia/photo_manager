...
function createSearchService({ modelsRoot, spawn }) {
  ...
  async function encode(query, signal) {
    ...
    const visual = await embeddings.encode('visualText', [query], signal);
    const text = (await embeddings.encode('queryMetadata', [query], signal))[0].map(c => c.vector);
    return { visual, description: text, context: text };
  }
