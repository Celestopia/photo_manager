...
Clicking a static-image or animated-GIF tile in the composer or a sent user message opens a centered, medium-size still preview over a blurred application backdrop. The preview preserves the complete image instead of the tile's square crop; GIFs show their first frame. Previous and Next move through previewable inputs in that composer draft or sent message without crossing into another message. The buttons disable at the first and last image rather than wrapping, Left Arrow and Right Arrow provide the same navigation while the modal is open, and a compact counter reports the current position. Video and text tiles are excluded. Click the backdrop or Close to dismiss the preview.
...
Image previews use a whole-window modal with a blurred dark backdrop and a thick black image border. The preview fits within 70% of the window and never enlarges beyond the locally generated, 1,280-pixel bounded derivative. Circular navigation buttons float outside the left and right edges without changing that image size. Large previews load on demand and remain cached only until the modal closes. Preview generation stays local and does not contact the model provider.
