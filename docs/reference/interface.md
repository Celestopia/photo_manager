...
The Batch Operation panel shows the selected item count and summed file size from the complete gallery selection. One icon-only toggle sits to the right of that summary: it selects the complete result by default, highlights when every result is selected, and clears the selection when pressed again. Selection management does not remain in the filter controls after the panel opens. Its header uses an accessible X close button to exit selection mode; compact bold Clear and Apply buttons retain their full action descriptions as tooltips. A pinned bottom action row places an Explorer-compatible Copy button at the left and a red permanent-delete bin at the right. Copy keeps the selection and panel open so the same set can be pasted or reused.
