...
Assistant metadata review preserves the viewer's draft boundary. Accept is disabled while the selected target has unsaved Metadata edits. During an accepted write the application locks editing, then refreshes the selected media, draft and gallery from the saved result. Switching Metadata/Assistant does not discard drafts. Proposal creation and Decline do not write customization fields; full interaction rules belong to the [chat interface](../agent/interface.md).
