...
The Batch Operation panel shows the selected item count and summed file size from the complete gallery selection. Its header uses an accessible X close button; compact bold Clear and Apply buttons retain their full action descriptions as tooltips. A pinned bottom action row places an Explorer-compatible Copy button at the left and a red permanent-delete bin at the right. Copy keeps the selection and panel open so the same set can be pasted or reused.
