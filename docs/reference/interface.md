...
Ordinary queries return the complete grouped result and its total, without registries or pagination. One metadata pass applies filters; matched records are enriched and sorted once. Library information retains its separate image/video counts. Renderer uses shallow collections, creates all cards, and relies on native `loading="lazy"`; future virtualization must preserve complete-result semantics. Increasing request IDs discard stale responses.

Title, File name, Description and Semantic search all submit only on Enter or Search; IME confirmation does not submit. The renderer separates editable draft text from the applied query, so filters, sorting and metadata refreshes never submit unfinished text. Semantic mode adds a Results control after Search (default 10, range 1–100, reset on library close), combines visible filters with local similarity matching, and returns a flat list in relevance order without date headings or pagination. Sort controls are replaced by a relevance label while semantic results are active. An applied-query label, Clear and in-flight Cancel make submitted state explicit. See [Semantic Search](semantic-search.md) for indexing and ranking contracts.
