...
Photos and covers use `ViewerImage.vue` and `use-viewer-image.js`. Main enrichment supplies `__viewerImageUrl` as derived renderer-only data. The `viewer-image` scheme streams the original photo or cached WebP directly to Chromium, without Base64 IPC transport or full Sharp decoding during normal display. Full decoding remains part of generated-cover validation and batch cache verification.

URLs contain a random per-window/library-session capability, MediaId, content hash and cover revision. The resolver accepts no arbitrary filesystem path and validates indexed sources and cache boundaries. Chromium can cache immutable image responses. Closing a window/library or starting maintenance revokes URLs and aborts active resource streams; maintenance completion refreshes descriptors even after failure. Deletion removes the indexed identity so subsequent requests fail. Repair increments the hash revision and updates duplicate-content descriptors. No persisted schema or compatibility layer is introduced.

The selected URL is available immediately. Loading and failures leave the same neutral stage for photos and covers, without an oversized video placeholder or the previous item's image. After the selected image loads, at most two adjacent images are preloaded within an estimated combined 64 MiB decoded-pixel budget, based on indexed dimensions. Unknown or oversized neighbors are skipped. Prefetching never generates covers. Selection/view/library changes and unmount release owned preload references; Chromium may retain its own cache independently.

A selected missing/damaged cover triggers at most one repair per selection. Request, media and library identity guard responses. Superseded requests cancel generation; failures stay quiet and may retry on a later selection. Late repairs cannot reset the video element or replace a decoded playback frame.
