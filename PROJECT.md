...
- Chatting about explicitly selected viewer media and imported attachments, with persistent library-local conversations and provider-reported per-completion token usage. Media navigation prepares a fresh draft; only submitted conversations appear in History (see the viewer chat reference below).
