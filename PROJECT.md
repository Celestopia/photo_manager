...
- `browser-api.js` 提供 `photoManagerApi` 的 mock 实现
...
- `app.js`：单文件承载主要前端逻辑（画廊 + 图像查看）
- `styles.css`：布局、动画、面板与控件样式
- `index.html`：Electron 入口
- `browser.html` + `browser-api.js`：浏览器预览入口与 mock API
- `assets/*.svg`：图标资源
...
- 引入 TypeScript，提升大文件（尤其 `app.js`）可维护性。
