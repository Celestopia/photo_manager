...
核心目标：
- 以本地图库文件、`photo_metadata.jsonl`、`tag_registry.jsonl` 和 `album_registry.jsonl` 为数据事实来源。
...
### 2.2 两种运行模式
1. Electron 模式（主模式）
- 真实读取 `config.yml`、`photo_metadata.jsonl`、`tag_registry.jsonl` 和 `album_registry.jsonl`
...
- `main.js` 职责：
  ...
  - 后台预热缩略图缓存（缺失项增量生成）
  - 注册 IPC 接口（查询、更新、批量更新、标签注册表、相册注册表、复制、窗口动作）
  - 创建无边框窗口并维护窗口状态同步
  - 记录运行日志，辅助白屏/崩溃诊断
...
5. 创建 BrowserWindow，加载 `index.html`。
6. 主进程后台启动缩略图缓存预热任务。
7. 渲染层 mounted 后请求配置、标签/相册注册表与首屏画廊数据。
...
5. 确认后调用 `photo:update-customization`。
6. 主进程校验相册/标签均已注册，合并 patch、更新时间戳并原子写回 JSONL。
7. 返回更新项，前端同步 `selectedItem` 与画廊缓存。

### 5.4 批量编辑流程（画廊多选）
1. 用户进入选择模式，选择多张图（支持全选/全不选）。
2. 右侧批量编辑卡片填写：标题、待设置相册、待添加标签、位置（国家/省/市/具体地点）。
