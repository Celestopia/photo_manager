...
三处共享层级构造数据，但交互语义不同：查看器、批量编辑和父节点选择器只能选择真实 `LocationId`；只有画廊顶部筛选器把国家、省、市派生分组行作为可选查询条件。`Location.Detail` 始终不参与地点或行政区筛选。
...
| `use-gallery-query.js` | 完整查询条件、筛选抽屉会话状态、日期分组、顺序媒体集合、统计、筛选项和 `MediaId` 索引 | 请求序号用于丢弃过期响应 |
