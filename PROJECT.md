...
核心目标：
- 以本地图库文件、`photo_metadata.jsonl`、`tag_registry.jsonl`、`album_registry.jsonl`、`person_registry.jsonl` 和 `location_registry.jsonl` 为数据事实来源。
...
- 标签注册表：JSONL（`tag_registry.jsonl`，每行一个标签定义）
- 相册注册表：JSONL（`album_registry.jsonl`，每行一个相册定义）
- 人物注册表：JSONL（`person_registry.jsonl`，每行一个人物定义，说明可为空）
- 地点注册表：JSONL（`location_registry.jsonl`，每行一个地点定义，说明可为空）
- 缩略图缓存：WebP 文件缓存（`thumb_cache/<SHA256Hash>.webp`）
- EXIF 解析：`exifr`

### 2.2 两种运行模式
1. Electron 模式（主模式）
- 真实读取 `config.yml`、`photo_metadata.jsonl`、`tag_registry.jsonl`、`album_registry.jsonl`、`person_registry.jsonl` 和 `location_registry.jsonl`
...
- `tag_registry.jsonl`：每行一个标签定义，包含标签文本、说明、创建时间、更新时间
- `album_registry.jsonl`：每行一个相册定义，包含相册标题、说明、创建时间、更新时间
- `person_registry.jsonl`：每行一个人物定义，包含姓名、可选说明、创建时间、更新时间
- `location_registry.jsonl`：每行一个地点定义，包含地点名、国家、省、市、父节点、可选说明、创建时间、更新时间
...
3. 主进程加载 `tag_registry.jsonl`，并为历史元数据中已使用但未注册的标签补齐默认说明。
4. 主进程加载 `album_registry.jsonl`，并为历史元数据中已使用但未注册的相册补齐默认说明。
5. 主进程加载 `person_registry.jsonl`，并为历史元数据中已使用但未注册的人物补齐定义（说明可为空）。
6. 主进程加载 `location_registry.jsonl`，将历史 `Location.Site` 迁移为 `Location.Place`，并补齐未注册地点。
7. 创建 BrowserWindow，加载 `index.html`。
8. 主进程后台启动缩略图缓存预热任务。
9. 渲染层 mounted 后请求配置、标签/相册/人物/地点注册表与首屏画廊数据。
...
4. 标签字段只能选择已注册标签；新标签必须先写入注册表并填写说明。
