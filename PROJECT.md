...
- `schemaVersion` 是说明图库数据契约的提示字段；当前新图库写入 `2`，运行时不根据该值选择解析、兼容或迁移逻辑。
...
选择模式中，选中至少一个媒体后显示批量元信息侧栏，可批量设置标题、Privacy、相册和主地点，并批量添加人物、标签。批量 Privacy 草稿以 `null` 表示“不修改”，等级 1 是独立的有效写入值；点击等级 N 后累计点亮前 N 个按钮，“清空输入”恢复为 `null`。焦点不在输入框、文本域、选择器或可编辑区域时，按 `Escape` 退出选择模式；若画廊右键详情或选择器下拉菜单处于打开状态，`Escape` 优先关闭当前浮层，不同时退出选择模式。
...
| `use-gallery-selection.js` | 选择模式、完整结果上的选中集合、含可选 Privacy 补丁的批量编辑草稿和批量结果 | 无文档级监听器 |
