...
核心目标：
- 以本地图库文件、`photo_metadata.jsonl` 和 `tag_registry.jsonl` 为数据事实来源。
...
### 2.2 两种运行模式
1. Electron 模式（主模式）
- 真实读取 `config.yml`、`photo_metadata.jsonl` 和 `tag_registry.jsonl`
...
- `main.js` 职责：
  ...
  - 后台预热缩略图缓存（缺失项增量生成）
  - 注册 IPC 接口（查询、更新、批量更新、标签注册表、复制、窗口动作）
  - 创建无边框窗口并维护窗口状态同步
  - 记录运行日志，辅助白屏/崩溃诊断
...
4. 创建 BrowserWindow，加载 `index.html`。
5. 主进程后台启动缩略图缓存预热任务。
6. 渲染层 mounted 后请求配置、标签注册表与首屏画廊数据。
...
3. 标签字段只能选择已注册标签；新标签必须先写入注册表并填写说明。
4. 确认后调用 `photo:update-customization`。
5. 主进程校验标签均已注册，合并 patch、更新时间戳并原子写回 JSONL。
6. 返回更新项，前端同步 `selectedItem` 与画廊缓存。
