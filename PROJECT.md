...
- `schemaVersion` 是说明图库数据契约的提示字段；当前新图库写入 `3`，运行时不根据该值选择解析、兼容或迁移逻辑。
...
- `FilePath`：媒体相对于图库根目录的当前位置，可以因移动或改名而变化，只用于定位文件、展示文件名和路径排序。
...
Privacy 当前只记录用户判断，不影响本地画廊可见性、筛选、排序、剪贴板、系统打开或 CSV 导出。主进程和维护脚本始终按当前结构处理元数据：Privacy 缺失、不是数字整数或超出 1 到 5 时，图库加载或相应维护操作失败并指出具体媒体；manifest 的 `schemaVersion` 不改变这套规则。
...
ID 创建后不可变。当前 UI 不支持标签、相册、人物或地点改名；未来实现改名时只修改显示字段，不需要重写媒体引用。四类注册表 ID 与 `MediaId` 共享同一个全局 UUID 命名空间。
...
1. 应用媒体类型筛选：全部、图片、视频。
2. 应用相册、标签、人物、具体地点或行政区筛选，多个维度取交集。
3. 应用文本搜索。
4. 排序。
5. 按拍摄日期分组。
...
| `use-gallery-query.js` | 完整查询条件、日期分组、顺序媒体集合、统计、筛选项和 `MediaId` 索引 | `thumbnail:ready` IPC 订阅；请求序号用于丢弃过期响应 |
